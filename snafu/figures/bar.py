...
def main(args):
	...
	for group_idx, group in enumerate(get_param(cfg, 'srcs')):
		...
		for bar_idx, bar in enumerate(group):
			...
			for key_idx, key in enumerate(get_param(stack, 'keys')):
				...
				running_height += (d / normalize) ** invert
			...
			if 'total' in stack:
				...
				ax.bar(minor_ticks[-1], d, 
					barwidth, running_height, color='grey', 
	...
	for i, ax in enumerate(axes):
		...
		ax.set_xticks([tick + 0.002 for tick in minor_ticks[1:]], minor=True)
		...
		if draw_norm:
			ax.plot(ax.get_xlim(), 
				[1.0, 1.0], 'k--', color='grey', linewidth=2.5)
			ax.margins(x=0.0)
	...
	if args.legend is not None:
		...
		if lbls.index('Remaining') != len(lbls) - 1:
