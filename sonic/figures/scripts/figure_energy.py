...
def main(args):
	...
	for d in data:
		...
		if d['avg_trial']['overall'] == 0:
			for i, ax in enumerate(axes):
				ax.plot(idx, 26 if i == 0 and args.zoom else 4, marker='x', markersize='8', 
		...
		for s in d['avg_trial']['sections']:
			if len(d['avg_trial']['sections']) == 2 and color_idx == 1 and len(config['colors']) == 5:
				color_idx += 1
			for ax in axes:
				ax.bar(idx, convert_energy(s['charge']),
					bottom=running_height,
					width=MAX_WIDTH * 0.9, 
					label=str(d[config['major_key']]) + '_' + str(d[config['minor_key']]), 
					color=config['colors'][color_idx])
			running_height += convert_energy(s['charge'])
			color_idx += 1;

		remaining_energy = convert_energy(d['avg_trial']['charge']) - running_height
		for ax in axes:
	...
	if args.legend:
		lbls = ['Live - Convolution', 
				'Live - Other', 
				'Dead (Recharging)', 
				'Does not Complete']
		hdls = [patches.Patch(label=lbls[i], color=stylecolors[i]) for i in range(0, 3)]
		...
		legfig.legend(handles=hdls, labels=lbls, 
			loc='center', ncol=4, columnspacing=1.5, handletextpad=0.25)
		legfig.tight_layout()
		if args.dest:
