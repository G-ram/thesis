...
def plots(args):
    ...
    plt.plot(x, 1e3 * transform(idealImage(x)), label='Ideal', ls='--')
    plt.plot(x, 1e3 * transform(naiveImage(x)), label='Naive local inference', ls='--')
    plt.plot(x, 1e3 * transform(sonicImage(x)), label='SONIC\&TAILS (this work)')
    # plotBrace(naiveImage(1.), idealImage(1.))
    # plotBrace(baseline(1.), naiveImage(1.))
    plotBrace(baseline(1.), idealImage(1.))
    plotBrace(naiveImage(1.), sonicImage(1.))
    plt.ylabel('Interesting images sent\nper harvested kilo-Joule')
    plt.xlabel('Accuracy')
    plt.xlim(0,1)
    plt.legend(loc='lower center', bbox_to_anchor=[0,1.02,1,1.1], ncol=2, columnspacing=1.5, handletextpad=0.25, fontsize=12)
    # improvementAxis = plt.gca().twinx()
    # improvementAxis.set_ylabel('Improvement vs. baseline', color='#B04040')
    # improvementAxis.tick_params('y', colors='#B04040')
    # improvementAxis.set_ylim(0, ideal(1.) / baseline(1.))
    plt.tight_layout(rect=[0,0,0.85,0.9])
    if args.dest: plt.savefig(args.dest + '_images.pdf')
    ...
    plt.plot(x, 1e3 * transform(naiveResult(x)), label='Naive local inference', ls='--')
    plt.plot(x, 1e3 * transform(sonicResult(x)), label='SONIC\&TAILS (this work)')
    plotBrace(sonicResult(1.), round(idealResult(1.), 2))
    plotBrace(baseline(1.), round(naiveResult(1.), 2))
    plotBrace(naiveResult(1.), round(sonicResult(1.), 2))
    plotBrace(baseline(1.), round(sonicResult(1.), 2), padding=0.15)
